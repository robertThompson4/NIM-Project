...
int clientMain(int argc, char *argv[], std::string playerName)
{
	...
	// Build an array of TicTacToe servers found on our subnet.
	...
	if (numServers == 0) {
		...
		// Display the list of TicTacToe servers found
		...
		// Allow user to select a TicTacToe server to challenge to a game
		int answer = 0;
		...
		if (answer >= 1 && answer <= numServers) {
			...
			int winner = playTicTacToe(s, serverName, host, port, PLAYER_X);
		}
	}
